...
export default function CourseDetailPage({ params }: { params: { slug: string } }) {
  ...
  const wizardSteps = isPublished
    ? [
        ...
        ...course.sections.map((section, idx) => ({
          ...
          previewBullets:
            section.bullets ||
            (section.code ? ["包含程式碼實作示範"] : undefined),
        })),
        ...(course.quiz
          ? [
              {
  ...
  return (
    <main className="mx-auto max-w-5xl px-5 py-14">
      ...
      <header className="mt-8 rounded-[2rem] border border-slate-200 bg-white p-8 shadow-soft md:p-12">
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          ...
          {course.source.demoUrl && (
            <a
              href={course.source.demoUrl}
              target="_blank"
              rel="noreferrer"
              className="rounded-full border border-slate-300 px-5 py-3 font-bold text-ink"
            >
              直接開啟原始 Demo
            </a>
          )}
      ...
      {!isPublished ? (
        <section className="mt-10 rounded-3xl border border-amber-200 bg-amber-50 p-7 md:p-9">
          <p className="text-sm font-black text-amber-700">CONTENT IN PROGRESS</p>
          <h2 className="mt-2 text-2xl font-black text-ink">
            課程內容仍在轉製中
          </h2>
          <p className="mt-4 max-w-3xl leading-8 text-slate-700">
            這門課尚未完成互動教學、Demo 與課後測驗，因此不標示為「可學習」。目前可先查看重點圖卡、課程簡報、NotebookLM 完整提示語、64 秒影片設計稿與來源程式。
          </p>
          <div className="mt-5 flex flex-wrap gap-2">
            {course.tags.map((tag) => (
              <span
                key={tag}
                className="rounded-lg bg-white px-3 py-2 text-sm font-medium text-slate-600 shadow-sm"
              >
                {tag}
              </span>
            ))}
        ...
        <>
          <section
            ...
            className="mt-10 grid scroll-mt-20 gap-5 md:grid-cols-2"
          >
            ...
            <section id="section-demo" className="mt-10 scroll-mt-20">
              <div className="mb-5">
                ...
                <p className="mt-2 text-slate-600">
                  展示模式由 Course Registry 統一管理，外部服務無法載入時仍可退回原始作品。
                </p>
              </div>
              <DemoAdapter
          ...
          <div className="mt-10 space-y-6">
            {course.sections.map((section, index) => (
              <section
                ...
                key={section.id}
                className="scroll-mt-20 rounded-3xl border border-slate-200 bg-white p-7 shadow-soft md:p-9"
              >
          ...
          {course.quiz && (
            <section
              id="section-quiz"
              className="mt-10 scroll-mt-20 rounded-3xl bg-ink p-8 text-white"
            >
