  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-soft transition-all duration-300 hover:-translate-y-1 hover:border-brand/30 hover:shadow-xl">
      ...
      <div className="relative aspect-[16/9] w-full overflow-hidden bg-slate-900/5">
        {course.coverImage ? (
          ...
          <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6 text-center text-white">
            <div>
              <span className="text-3xl font-black opacity-20">{course.category}</span>
              <p className="mt-1 text-sm font-semibold opacity-70">{course.title}</p>
            </div>
          </div>
        )}
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <p className="mt-1.5 text-xs font-bold text-brand tracking-wide">{course.subtitle}</p>
        <p className="mt-3 flex-1 text-sm leading-6 text-slate-600 line-clamp-3">{course.summary}</p>

        {/* 標籤 */}
        <div className="mt-4 flex flex-wrap gap-1.5">
          {course.tags.slice(0, 4).map((tag) => (
            <span key={tag} className="rounded-lg border border-slate-200 bg-slate-50/50 px-2 py-0.5 text-xs font-medium text-slate-500">
              #{tag}
            </span>
          ))}
        </div>
        ...
        {/* 底部導覽列 */}
        <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4 text-sm font-medium">
