...
def analyze():
    ...
    # 2. 偵測 Python 專案
    elif os.path.exists("requirements.txt") or os.path.exists("requirement.txt") or os.path.exists("pyproject.toml"):
        project_info["language"] = "Python"
        req_file = "requirements.txt" if os.path.exists("requirements.txt") else ("requirement.txt" if os.path.exists("requirement.txt") else None)
        if req_file:
            project_info["install_command"] = f"pip install -r {req_file}"
            try:
